...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall Aegis and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g aegis-ai",
      pnpm: "pnpm uninstall -g aegis-ai",
      bun: "bun remove -g aegis-ai",
      yarn: "yarn global remove aegis-ai",
      brew: "brew uninstall aegis",
      choco: "choco uninstall aegis",
      scoop: "scoop uninstall aegis",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall aegis -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using Aegis!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# aegis") || content.includes(".aegis/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".aegis/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".aegis/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".aegis"))
    ) {
      continue
    }
