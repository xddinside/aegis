...
const AGENT_USERNAME = "aegis-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/aegis.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/aegis summarize` to see the agent in action",
                "",
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/aegis-agent"
            const command =
...
jobs:
  aegis:
    if: |
      ...
      contains(github.event.comment.body, ' /aegis') ||
      startsWith(github.event.comment.body, '/aegis')
    ...
    steps:
      ...
      - name: Run aegis
        uses: anomalyco/aegis/github@latest${envStr}
        with:
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      try {
        ...
        console.log("aegis session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/aegis,/ag")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to aegis...")
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `aegis/${type}-${hex}-${timestamp}`
        }
        return `aegis/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        ...
        const shareUrl = shareId ? `[aegis session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the aegis infrastructure after your response",
