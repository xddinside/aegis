...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run Aegis",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for Aegis session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found Aegis session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`aegis import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting Aegis...")
        UI.println()

        // Launch Aegis TUI with session ID if available
        const { spawn } = await import("child_process")
        const aegisArgs = sessionId ? ["-s", sessionId] : []
        const aegisProcess = spawn("aegis", aegisArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          aegisProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`aegis exited with code ${code}`))
          })
          aegisProcess.on("error", reject)
        })
      },
    })
  },
